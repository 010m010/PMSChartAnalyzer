    ...
    def _build_explanation_tab(self) -> QWidget:
        ...
        sections = [
            (
                ...
                dedent(
                    """
                    <p>譜面の 1 ノーツ目を時刻 0 s とし、<b>1 秒ごと</b>にノーツ数を集計します。</p>
                    <ul>
                      <li>区間 t の秒間密度 <b>n<sub>t</sub></b> = 区間 [t, t+1) に含まれるノーツ数</li>
                      <li>n<sub>t</sub> = 0 の区間はプレイに影響しないため、平均系の計算から除外します</li>
                    </ul>
                    """
                ),
            ),
            (
                ...
                dedent(
                    """
                    <p>非ゼロ区間だけを対象に、次の 2 種類の代表値を計算します。</p>
                    <ul>
                      <li><b>平均密度</b>：算術平均。<code>avg = Σ n<sub>t</sub> / |T<sub>nz</sub>|</code></li>
                      <li><b>体感平均密度 (CHM)</b>：高密度に重み付けする反調和平均。<code>chm = Σ n<sub>t</sub><sup>2</sup> / Σ n<sub>t</sub></code></li>
                    </ul>
                    <p>体感平均密度は高密度区間へ強く反応するため、休憩地帯が多い譜面ほど平均密度との差が大きくなりやすいです。</p>
                    """
                ),
            ),
            (
                ...
                dedent(
                    """
                    <p>「終端」は、譜面の末尾側からノーツを積算し、beatoraja 9key モードのクリアゲージが
                    初期値 <b>2</b> から基準値 <b>85</b> に到達するために必要なノーツ数を満たす範囲を 1 秒区間単位で切り出したものです。</p>
                    <p>単曲分析画面では、この終端範囲をグラフ上のハイライト領域として表示します。</p>
                    """
                ),
            ),
            (
                ...
                dedent(
                    """
                    <p>終端として切り出した区間に対し、平均密度と体感平均密度 (CHM) を同じ手順で計算したものです。</p>
                    """
                ),
            ),
            (
                ...
                dedent(
                    """
                    <p>曲全体に対し、<code>chm</code> を小数点切り捨てしたしきい値以上の秒間密度が出る区間の割合です。</p>
                    <p><code>occupancy = count(n<sub>t</sub> ≥ ⌊chm⌋) / |T| × 100</code></p>
                    <p>値が高いほど全体難、低いほど局所難の傾向を示します。</p>
                    """
                ),
            ),
            (
                ...
                dedent(
                    """
                    <p>秒間密度の変化幅を、ノーツ総数で正規化した指標です。</p>
                    <p><code>dchange = Σ |n<sub>t</sub> − n<sub>t−1</sub>| / (NOTES + ε)</code></p>
                    <p>高密度占有率と組み合わせることで、秒間密度チャートの形状を推測できます。</p>
                    """
                ),
            ),
            (
                ...
                dedent(
                    ...
                    <table border=\"1\" cellspacing=\"0\" cellpadding=\"4\">
                      <thead>
                        <tr><th>高密度占有率</th><th>密度変化量</th><th>傾向</th></tr>
                      </thead>
                      <tbody>
                        <tr><td>高</td><td>高</td><td>高密度と低密度が短周期で持続</td></tr>
                        <tr><td>高</td><td>低</td><td>高密度と休憩のメリハリが明確、またはフラット</td></tr>
                        <tr><td>低</td><td>高</td><td>高密度の局所発狂が複数回（低密度も揺れる場合あり）</td></tr>
                        <tr><td>低</td><td>低</td><td>非常に高い密度の局所発狂と長い低密度区間</td></tr>
                      </tbody>
                    </table>
                    """
                ),
            ...
            (
                ...
                dedent(
                    """
                    <p>最大秒間密度が平均値からどれだけ突出しているかを示す指標です。</p>
                    <p><code>gustiness = (max − avg) / (std + ε)</code></p>
                    """
                ),
            ),
            (
                ...
                dedent(
                    """
                    <p>終端体感密度と非終端体感密度の差です。値が大きいほどラス殺し寄り、小さいほどラスト回復寄りになります。</p>
                    <p><code>terminal_diff = chm<sub>terminal</sub> − chm<sub>non-terminal</sub></code></p>
                    """
                ),
            ),
            (
                ...
                dedent(
                    """
                    <ul>
                      <li><b>全体難度数</b>：平均密度を標準偏差で割った値。密度のばらつきに対する相対的な平均の高さを示します。</li>
                      <li><b>終端突風度数</b>：終端範囲のみを対象とした突風度数。</li>
                    </ul>
                    """
                ),
            ),
